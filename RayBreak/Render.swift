...
extension Render {
    private struct Uniforms {
        ...
        var grayShader:Int
        var invertedShader:Int
        var invertedShader2:Int
        
    }
    
    private struct LightShades {
        var redShade:Float
        var greenShade:Float
        var blueShade:Float
    }
    
    private struct ShadesOn {
        var lightsShade:Float
    }
...
class Render: NSObject, MTKViewDelegate {
    ...
    private let shadesOnBuffer: MTLBuffer
    ...
    var activeShaders:[Int] = [0,0,0,0]
    var redShade:Float = 0.5
    var greenShade:Float = 0.5
    var blueShade:Float = 0.5
    var lightShade:Float = 0.0
    ...
    init(view: MTKView) throws {
        ...
        let descriptors = try Render.makeStateDescriptors(device: device, pixelFormats: pixelFormats)
        ...
        self.meshes = try Render.makeMeshes(device: device, vertexDescriptor: descriptors.renderPipeline.vertexDescriptor!)
        (self.diffuseTexture, self.samplerTexture) = try Render.makeTexture(device: device)
        ...
        guard let lightBuffer = device.makeBuffer(length: MemoryLayout<LightShades>.stride) else { throw Error.failedToCreateMetalBuffer(device: device) }
        guard let shadesOnBuffer = device.makeBuffer(length: MemoryLayout<ShadesOn>.stride) else { throw Error.failedToCreateMetalBuffer(device: device) }
        uniformBuffer.label = "label"
        uniformBuffer.contents().bindMemory(to: Uniforms.self, capacity: 1)
        lightBuffer.label = "light.buffer"
        lightBuffer.contents().bindMemory(to: LightShades.self, capacity: 1)
        shadesOnBuffer.label = "shadesOn.buffer"
        shadesOnBuffer.contents().bindMemory(to: ShadesOn.self, capacity: 1)
        self.uniformsBuffer = uniformBuffer
        self.lightsBuffer = lightBuffer
        self.shadesOnBuffer = shadesOnBuffer
        // Setup the MTKView.
        view.setUp {
    ...
    func scaleToGray(){
        activeShaders[0] = activeShaders[0] == 0 ? 1 : 0
    }
    func invertImage(){
        activeShaders[1] = activeShaders[1] == 0 ? 1 : 0
    }
    ...
    func draw(in view: MTKView) {
        ...
        do {
            ...
            encoder.setVertexBuffer(self.lightsBuffer, offset: 0, index: 2)
            encoder.setVertexBuffer(self.shadesOnBuffer, offset: 0, index: 3)
    ...
    private static func makeStateDescriptors(device: MTLDevice, pixelFormats: PixelFormats) throws -> (renderPipeline: MTLRenderPipelineDescriptor, depthStencil: MTLDepthStencilDescriptor) {
        ...
        let renderPipelineDescriptor = MTLRenderPipelineDescriptor().set { (pipeline) in
            ...
            pipeline.vertexDescriptor = MTLVertexDescriptor().set {
                ...
                $0.attributes[3].setUp { (attribute) in
                    attribute.bufferIndex = 0
                    attribute.offset = MemoryLayout<Float>.stride * 9
                    attribute.format = .int
                }
                $0.attributes[4].setUp { (attribute) in
                    attribute.bufferIndex = 0
                    attribute.offset = MemoryLayout<Float>.stride * 10
                    attribute.format = .int
                }

                $0.layouts[0].stride = MemoryLayout<Float>.stride * 11
            }
...
//            ($0.attributes[3] as! MDLVertexAttribute).name = "MDLVertexColorScale"
        }
...
//        self.rotationY += duration * (.𝝉 / 6.0)
        ...
        let xRotMatrix  = float4x4(rotate: SIMD3<Float>(1, 0, 0), angle: -self.rotationY)
...
//        let modelMatrix = scaleMatrix
        ...
        ptr.pointee = Uniforms(modelViewProjectionMatrix: modelViewProjectionMatrix, modelViewMatrix: modelViewMatrix, normalMatrix: normalMatrix, grayShader: self.activeShaders[0], invertedShader: self.activeShaders[1], invertedShader2: 1)
        let ptr2 = lightsBuffer.contents().assumingMemoryBound(to: LightShades.self)
        ptr2.pointee = LightShades.init(redShade: redShade, greenShade: greenShade, blueShade: blueShade)
        let ptr3 = shadesOnBuffer.contents().assumingMemoryBound(to: ShadesOn.self)
        ptr3.pointee = ShadesOn.init(lightsShade: lightShade)
    }
}
